...
export class CheckinComponent implements OnInit {
    ...
    this.dtenv.get(Endpoint.GetRoomstobook + this.rateID + '/' + this.ratetypeID)
      .subscribe((res: any) => {
        ...
        if (this.Rooms.length && this.formatedInDate && this.formatedOutDate) {
          this.dtenv.get(Endpoint.gettoBookRoom + this.Rooms[0].RoomId + "/1/" + this.formatedInDate + "/" + this.formatedOutDate)
            .subscribe((res: any) => {
  ...
  onDateSelect(date: NgbDateStruct, type: string) {
    ...
    if (type === "checkin") {
      this.CheckinDate = new Date(formatedDate);
      this.formatedInDate = formatedDate;
      this.minOutDate = { year: date.year, month: date.month, day: date.day };
    } else {
      this.CheckoutDate = new Date(formatedDate);
      this.formatedOutDate = formatedDate;
    }

    if (this.CheckinDate && this.CheckoutDate) {

      this.dtenv.get(Endpoint.gettoBookRoom + this.Rooms[0].RoomId + "/1512/" + this.formatedInDate + "/" + this.formatedOutDate)
        .subscribe((res: any) => {
          this.room_details = res.pop();
        });

      const dates = this.getDates(this.CheckinDate, this.CheckoutDate);
      if (dates.length) {
