...
export class DetailComponent {
  ...
  getData(): void {
    ...
    this.dtenv.get(Endpoint.gettoBookRoom + this.RoomID + "/1/" + formatedInDate + "/" + formatedOutDate)
      .subscribe((res: any) => {
        ...
        this.dtenv.get(Endpoint.gettoBookBuldingrooms + this.room.BuldingID)
          .subscribe((res) => {
            this.dtenv.get(Endpoint.GetRoomImageList + res[0].RoomId)
              .subscribe((res) => {
                this.roomImagePath = this.resourcePath + "rooms/1/" + res[0].Imagepath;
              });
          })
  ...
  submit() {
    const invoiceHtml: any = document.getElementById("invoice-container")?.innerHTML;
